...
func runUpgrade(cmd *cobra.Command, args []string, opts *upgradeOptions) error {
	...
	if !opts.planOnly && result.Applied {
		log.Info("pushing release state to OCI", "version", newVersion)
		...
		releaseState := &state.ReleaseState{
			Metadata: state.NewReleaseMetadata(releaseName, moduleRef, newVersion),
			MainTF:   mainTF,
			TFState:  tfState,
			Values:   valuesYAML,
		}
		...
		// Build new state reference (flat format: repo:releaseName-version)
		newStateRef := fmt.Sprintf("%s:%s", stateRepo, state.FormatReleaseTag(releaseName, newVersion))

		// Push state to OCI
		if err := stateStore.PushState(ctx, newStateRef, releaseState); err != nil {
