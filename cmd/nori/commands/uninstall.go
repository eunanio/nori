...
func newReleaseUninstallCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "uninstall <name>",
		Aliases: []string{"delete", "del", "un", "destroy"},
		Short:   "Uninstall a release",
		Long: `Uninstall a release and destroy its infrastructure.
...
Similar to Helm's uninstall command.

Examples:
  # Uninstall a release (will prompt for confirmation)
  nori release uninstall my-bucket

  # Uninstall with auto-approve
  nori release uninstall my-bucket --auto-approve

  # Dry run (show what would be destroyed)
  nori release uninstall my-bucket --dry-run

  # Keep the release history (only destroy resources)
  nori release uninstall my-bucket --keep-history`,
...
func runUninstall(cmd *cobra.Command, args []string, opts *uninstallOptions) error {
	...
	log.Info("uninstalling release", "name", releaseName, "module", rel.ModuleRef)

	// Update status
	rel.Status = release.StatusUninstalling
	...
	if !opts.keepHistory {
		...
		fmt.Printf("release %q uninstalled\n", releaseName)
	} else {
		// Mark as uninstalled but keep record
		rel.Status = release.StatusFailed // Use failed as "uninstalled" state
		if err := store.Save(rel); err != nil {
			log.Warn("failed to update release status", "error", err)
		}
