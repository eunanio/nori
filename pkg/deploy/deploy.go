...
// generateTofuCLIConfig creates an OpenTofu CLI config file with credential helper settings.
// OpenTofu does not use Docker credential helpers by default, so we must explicitly configure it.
// See: https://opentofu.org/docs/cli/oci_registries/credentials/
func (d *Deployer) generateTofuCLIConfig(workDir string) (string, error) {
	helper := auth.GetCredentialHelper()
	if helper == "" {
		d.logger.Debug("no credential helper available for this platform")
		return "", nil
	}

	// Ensure we use an absolute path for the config file
	absWorkDir, err := filepath.Abs(workDir)
	if err != nil {
		return "", fmt.Errorf("failed to get absolute path: %w", err)
	}

	// Generate oci_default_credentials block to enable Docker credential helper for all registries
	config := fmt.Sprintf(`# Auto-generated by nori for OpenTofu OCI registry authentication
# This configures OpenTofu to use the Docker credential helper for the current platform

oci_default_credentials {
  docker_credentials_helper = %q
}
`, helper)

	configPath := filepath.Join(absWorkDir, ".tofurc")
	if err := os.WriteFile(configPath, []byte(config), 0644); err != nil {
		return "", fmt.Errorf("failed to write tofu config: %w", err)
	}

	d.logger.Debug("generated OpenTofu CLI config", "path", configPath, "helper", helper)
	return configPath, nil
}

// buildBaseEnv creates the base environment for terraform commands,
// filtering out TF_CLI_CONFIG_FILE to avoid conflicts with our generated config.
...
func (d *Deployer) runTerraformWithExitCode(ctx context.Context, workDir string, args []string, stdout, stderr io.Writer) error {
	...
	cmd.Stdout = stdout
	cmd.Stderr = stderr
	cmd.Env = buildBaseEnv()

	// Generate OpenTofu CLI config with credential helper for OCI registry authentication
	if configPath, err := d.generateTofuCLIConfig(workDir); err == nil && configPath != "" {
		cmd.Env = append(cmd.Env, "TF_CLI_CONFIG_FILE="+configPath)
	} else if err != nil {
		d.logger.Warn("failed to generate tofu config", "error", err)
	}
